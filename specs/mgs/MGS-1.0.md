...
**Implementation note (see §9):** a Checker
...
This is a direct, deliberate rejection of a defect pattern common to
audit tools that compute a single aggregate score: a Dataset with zero
images can produce a deceptively high score if every missing metric
silently defaults to a neutral value instead of signaling that nothing
was actually measured. Under MGS-0000, the same input MUST produce
...
documented, defensible justification yet. Promoting it to a normative
Requirement is deferred to a future MGS version once a threshold can be
justified the way §5.2–5.4 are.
...
**MGS 1.0 is frozen.** It was carried as a draft while building the
reference implementation's Reader/Manifest/Checker pipeline, revised by
findings made along the way, and frozen once the conformance corpus
(§7.2) — `conformance/fixtures/` + `conformance/expected/*.json`, run
via `conformance/runner.py` — went green across all fixtures. Per §8,
further changes to §5 (Requirements) now constitute MGS 1.1 or later,
not edits to this document.

**Revisions made while it was still a draft:**
