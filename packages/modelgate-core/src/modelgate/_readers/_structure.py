...
This is the single implementation deciding (label, split) per file.
Having exactly one implementation, used by every Reader, is what
guarantees a flat-class layout (`Cat/`, `Dog/` directly at the root)
and a single-root layout (`PetImages/Cat/`, `PetImages/Dog/`) with
identical logical content produce identical Manifests — see
ARCHITECTURE.md.
"""
