...
The `check` subcommand exists because the conformance corpus (spec
§7.3) defines conformance through a process-level CLI contract —

    <tool> check <fixture-dir> --json

— and `--spec` version pinning (spec §8's requirement that a caller
can pin an exact spec version, and that the tool refuses rather than
silently evaluating against a different one).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "check":
        ...
        # `modelgate check` to be usable as a CI gate.
