...
Gini coefficient over per-label sample counts, using the Manifest's
classified samples (images only) — a stray non-image file in a class
directory can't shift the coefficient, since it's never a Sample at
all (see `_readers/_structure.py`'s extension filter).
"""
